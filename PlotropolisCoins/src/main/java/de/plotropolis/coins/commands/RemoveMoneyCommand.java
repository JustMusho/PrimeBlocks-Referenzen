...
public final class RemoveMoneyCommand implements CommandExecutor, TabCompleter {

    private final PlotropolisCoins plugin;

    public RemoveMoneyCommand(PlotropolisCoins plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!sender.hasPermission("plotropoliscoins.admin.removemoney")) {
        ...
        plugin.getDataStore().removeMoney(t.getUniqueId(), name, amount);

        send(sender, "admin-removed-money",
                "{player}", name,
