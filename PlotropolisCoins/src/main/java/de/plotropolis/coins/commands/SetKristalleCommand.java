...
public final class SetKristalleCommand implements CommandExecutor, TabCompleter {

    private final PlotropolisCoins plugin;

    public SetKristalleCommand(PlotropolisCoins plugin) {
        this.plugin = plugin;
    }
    ...
    private Long parseNonNegative(String in) {
        try {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!sender.hasPermission("plotropoliscoins.admin.setkristalle")) {
        ...
        Long amount = parseNonNegative(args[1]);
        if (amount == null) {
            send(sender, "invalid-number");
            return true;
        }
        ...
        plugin.getDataStore().setKristalle(t.getUniqueId(), name, amount);

        send(sender, "admin-set-kristalle",
                "{player}", name,
                "{amount}", MoneyFormat.formatKristalle(amount)
        );
        return true;
    }
